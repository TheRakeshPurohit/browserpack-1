...
export default function Breadcrumbs({ filePath }: BreadcrumbsProps) {
  ...
  return fileParts && fileParts.length > 0 ? (
    <Container>
      {fileParts.map((filePart, index) => {
        ...
        return (
          <BreadcrumbContainer key={index}>
            {isFileName && <Icon entityName={filePart} />}
            <div style={{ marginLeft: isFileName ? '5px' : 0 }}>{filePart}</div>
            {!isFileName && <div className="codicon codicon-chevron-right" />}
          </BreadcrumbContainer>
        );
      })}
    </Container>
