...
const SectionHeader = styled.div`
  ...
  padding-left: 0;
  background: ${(props) =>
    props.theme.colors['sideBarSectionHeader.background']};
  color: ${(props) => props.theme.colors['sideBarSectionHeader.foreground']};
  border-bottom: 1px solid
    ${(props) => props.theme.colors['sideBarSectionHeader.border']};
  display: flex;
  cursor: pointer;
`;
...
const Section = ({ title, children, defaultOpen }: SectionProps) => {
  ...
  return (
    <div>
      ...
      <SectionContent isOpened={isOpen || false}>{children}</SectionContent>
    </div>
  );
};
...
export default function SideBar({ selectedContainer = 'files' }: SideBarProps) {
  return (
    <>
      ...
      <Container>
        <div
          style={{ display: selectedContainer === 'files' ? 'block' : 'none' }}
        >
        ...
        <div
          style={{ display: selectedContainer === 'search' ? 'block' : 'none' }}
        >
          <Header>Search</Header>
        </div>
        <div
          style={{ display: selectedContainer === 'scm' ? 'block' : 'none' }}
        >
          <Header>Search</Header>
        </div>
        <div
          style={{
            display: selectedContainer === 'extensions' ? 'block' : 'none'
          }}
        >
